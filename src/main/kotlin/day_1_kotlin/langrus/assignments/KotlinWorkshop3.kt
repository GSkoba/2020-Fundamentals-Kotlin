...
object KotlinWorkshop3 {
    ...
    private fun printTotalCountOfGuesses(guessCounter: Int) {
        println("Число попыток: $guessCounter")
    }

    // TODO 4
    private fun IntArray.countHigherGuesses(guesses: IntArray, randomNumber: Int): Int {
        val countHigher = guesses.filter { it > randomNumber }.count()
        println("Больше загаданного числа: $countHigher")
        return countHigher
    }

    // TODO 5
    val countLowerGuesses: (IntArray, Int) -> Unit = { guesses, randomNumber ->
        val countLower = guesses.filter { it < randomNumber && it != -1 }.count()
        println("Меньше загаданного числа: $countLower")
    }
